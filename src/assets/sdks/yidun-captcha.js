...
(function(global, factory) {
  ...
  function normalizeFallbackConfig(customConfig) {
    ...
    const config = ObjectAssign({}, defaultConf, customConfig)

    const errorFallbackCount = config.errorFallbackCount
    assert(
  ...
  */
  function initNECaptchaWithFallback(options, onload, onerror) {
    ...
    const fallbackFn = !defaultFallback ? config.onFallback || function() {} : function(validate) {
    ...
    const proxyOnError = function(error) {
      errorCallbackCount++
    ...
    const triggerInitError = function(error) {
